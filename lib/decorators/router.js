...
exports.Middleware = (convert) => {
    return (...args) => utils_1.Decorate(args, convert);
};
const validateAndThrow = (type, ctx, data, schema) => {
    const validateResult = v.validate(data, schema);
    if (!validateResult.valid) {
        validateResult.instance;
        ctx.throw(412, `${type} validation error: ${validateResult.errors.map(e => `${e.property.replace('instance.', '')} ${e.message.replace('instance ', type)}`).join(', ')}`, validateResult.errors);
    }
};
exports.Required = (rules) => {
    return (...args) => {
        return utils_1.Decorate(args, (ctx, next) => __awaiter(this, void 0, void 0, function* () {
